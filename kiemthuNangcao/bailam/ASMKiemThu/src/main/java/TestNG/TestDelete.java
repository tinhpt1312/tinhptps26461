...
public class TestDelete {
	...
	private void readDataFromExcel() {
		try {
			...
			worksheet=workbook.getSheet("TestDelete");
			if(worksheet ==null) {
	...
	public void deleteTest() {
		try {
			...
			for (String key : keySet) {
				...
				Thread.sleep(1000);
				WebElement btnDelete = driver.findElement(By.name("delete"));
				...
				if (tt.getAttribute("value").equalsIgnoreCase(expected)) {
					TestNGResults.put(String.valueOf(index + 1),
							new Object[] { String.valueOf(index), username, password, fullname, email, "Test Delete",
			...
			// TODO: handle exception
			System.out.println("Delete Test(): " + e.getMessage());
		}
	}
	...
	public void suiteTearDown() {
		...
		for (String key : keySet) {
			...
			try {
				FileOutputStream out = new FileOutputStream(new File(excelDir + "RESULT_TEST_Delete.xlsx"));
				workbook.write(out);
				out.close();
