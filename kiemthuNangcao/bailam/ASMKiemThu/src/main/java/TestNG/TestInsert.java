...
public class TestInsert {
		...
		try {
			...
			for(String key: keySet) {
				...
				if(tt.getAttribute("value").equalsIgnoreCase(expected)) {
					TestNGResults.put(String.valueOf(index+1), new Object[] {String.valueOf(index),username,
							password,fullname,email,"Test Insert",expected,tt.getAttribute("value"),"Pass",formattedDate
				});
				}else {
					System.out.println("Test Fail!!");
					TestNGResults.put(String.valueOf(index+1), new Object[] {String.valueOf(index),username,
							password,fullname,email,"Test Insert",expected,tt.getAttribute("value"),"Failed",formattedDate
				});
				}
			index++;
			}
	...
	public void suiteTearDown() {
		...
		for (String key : keySet) {
			...
			try {
				FileOutputStream out = new FileOutputStream(new File(excelDir + "RESULT_TEST_INSERT.xlsx"));
				workbook.write(out);
				out.close();
